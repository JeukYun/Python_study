...
'''
    ...
    1. 기존 파이썬에서 지원 안된 요소간의 연산을 numpy는 지원.
    2. 다차원 배열간 산술연산, 다차원 배열과 스칼라 산술연산
    3. 비교연산도 가능 ==> 논리값으로 결과가 출력 (True / False) ==> boolean 색인  ( 중요 )  
...
# 2. 다차원 배열 + 다차원 배열 ==> 요소간 연산
# 반드시 shape 일치해야 된다. / 안맞는 경우 오류 발생
...
# 3. 백터 + 스칼라 ==> 자동으로 브로드캐스팅(차원을 맞춰줌) 되어 연산됨.
'''
[10,20,30] +2 인경우
==>> [10,20,30] + [2, 2, 2]로 브로드캐스팅
'''
arr1D_1 = np.array([10,20,30])
